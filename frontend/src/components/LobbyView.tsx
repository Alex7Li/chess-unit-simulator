      ...
      return boardLine.map((boardCell, col) => {
        ...
        if (boardCell != null) {
          ...
          if (boardCell.isRoyal) {
            inner_royal = <img draggable="false" src="static/crown.png" className="absolute top-0 left-0 h-3 w-3 rotate-12 mx-4 -my-1 rounded-md drop-shadow-md"/>
          }
        }
...
interface LobbyProps {
  lobby: RefObject<WebSocket>
}
export const LobbyView: FC<LobbyProps> = ({lobby}) => {
  ...
  return <div>
    ...
    <div className="inline-block">
      {lobbySetups.map((lobbySetup) => {
        const requestGameClicked = () => {
          ...
          lobby.current.send(JSON.stringify(
            {
        ...
        return <button key={lobbySetup.pk} className="col-span-1 m-2 bg-neutral-200" onClick={requestGameClicked}>
    ...
    <div className="inline-block">
      {boardSetups.map((boardSetup) => {
        ...
        return <button key={boardSetup.pk} className="col-span-1 m-2 bg-neutral-200" onClick={requestGameClicked}>
